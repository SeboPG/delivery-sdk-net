...
namespace KenticoCloud.Delivery.Tests.Extensions
{
    public class ServiceCollectionsExtensionsTests
    {
        ...
        public void AddDeliveryClientWithConfiguration_AllServicesAreRegistered(string fileNamePostfix,
            string customSectionName = null)
        {
            ModifyExpectedOptionsImplementationTypes();

            var jsonConfigurationPath = Path.Combine(Environment.CurrentDirectory, "Fixtures",
                "ServiceCollectionsExtensions", $"deliveryOptions_{fileNamePostfix}.json");
